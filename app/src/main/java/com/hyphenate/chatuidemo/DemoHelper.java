...
//==========================================================================================
        ...
        if(username.equals(EMClient.getInstance().getCurrentUser())){
            user=new EaseUser(username);
            user.setAvatar(SharedPreferencesUtil.getInstance().getUserHeadImg());
            user.setNick(SharedPreferencesUtil.getInstance().getUserName());
            return user;
        }
//        if (user==null && getRobotList()!=null){
//            user=getRobotList().get(username);
//        }
        ...
        if(user == null){
            ...
            if (StringUtil.isEmpty(user.getAvatar())){//如果名字为空，则显示环信号码
...
//                    int num = 0;
//                    for(int i = 0; i < ConversationListFragment.listEntity.getAvatarNickNameList().size(); i++) {
//                        if(hxIdFrom.equals(ConversationListFragment.listEntity.getAvatarNickNameList().get(i).getUserId())) {
//                            num = num + 1;
//                            break;
//                        }
//                    }
//                    if(num == 0) {
//                        ConversationListFragment.listEntity.getAvatarNickNameList().add(new AvatarNickNameEntity(hxIdFrom, userPic, userName));
//                        ConversationListFragment.mCache.put(Constant.AVATAR_NICKNAME_LIST, ConversationListFragment.listEntity);
//                    }
                }
			}
